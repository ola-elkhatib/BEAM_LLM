...
entity2label_path= '../../Data/Graph_data/Freebase/entity2label.txt'
pg_relation_prune_file_path='relation_prune_cache.gpickle'
dictionary_relation_prune_file_path="dictionary_relation_prune_cache.pickle"
full_relation_prune_file_path='full_relation_prune_cache.gpickle'
...
def get_cached_reponses():
    if os.path.exists(pg_relation_prune_file_path):
        with open(pg_relation_prune_file_path, 'rb') as f:
            PG_relation_prune_cache = pickle.load(f)
    else:
        PG_relation_prune_cache={}


    if os.path.exists(full_relation_prune_file_path):
        with open(full_relation_prune_file_path, 'rb') as f:
            full_relation_prune_cache = pickle.load(f)
    else:
        full_relation_prune_cache={}
    ...
    if os.path.exists(dictionary_relation_prune_file_path):
        with open(dictionary_relation_prune_file_path, 'rb') as f:
            dictionary_relation_prune_cache = pickle.load(f)
    else:
        dictionary_relation_prune_cache={}

    return [dictionary_relation_prune_cache, 0],[PG_relation_prune_cache, 0],[full_relation_prune_cache,0], [generate_answer_cache,0], [reasoning_cache,0], [force_answer_cache,0]

def save_responses(dictionary_relation_prune_cache, pg_relation_prune_cache,full_relation_prune_cache, generate_answer_cache, reasoning_cache, force_answer_cache) :
    with open(pg_relation_prune_file_path, 'wb') as f1:
        pickle.dump(pg_relation_prune_cache,f1)
    with open(full_relation_prune_file_path, 'wb') as f5:
        pickle.dump(full_relation_prune_cache,f5)
    ...
    with open(dictionary_relation_prune_file_path, 'wb') as f5:
        pickle.dump(dictionary_relation_prune_cache,f5)
...
def dictionary_relation_search_prune_embed(relation_prune_cache_list,entity_id, entity_name,relation_list, question, cfg, LLM_calls):
    LLM_calls+=1
    if len(relation_list)>3:
        if (question, entity_name, tuple(relation_list)) in relation_prune_cache_list[0]:
                result=relation_prune_cache_list[0][(question, entity_name, tuple(relation_list))]
        else:
            prompt = construct_relation_prune_prompt(question, entity_name, relation_list, cfg)
            result = run_llm(prompt, cfg.temperature_exploration, cfg.max_length, cfg.opeani_api_keys, cfg.LLM_type)
            relation_prune_cache_list[0][(question, entity_name, tuple(relation_list))]=result
            relation_prune_cache_list[1]+=1
        ...
        for rel in relation_list:
            relations.append({"entity": entity_id, "relation": rel, "score": 1, "head": True})
...
def relation_search_prune_embed(relation_prune_cache_list, beam_data,entity_id, entity_name,relation_list, question, cfg,prev_rel,hop, LLM_calls):
    ...
    if len(head_relations)>3:
        ...
        for rel in head_relations:
            relations.append({"entity": entity_id, "relation": rel, "score": relation_score_dict[rel], "head": True})
        return relations, LLM_calls
