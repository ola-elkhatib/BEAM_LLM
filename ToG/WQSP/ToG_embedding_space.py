...
def ToG_on_Embedding_Space(cfg,entity2idx,datas,nx_graph,rel2idx,model,device):
    ...
    dictionary_relation_prune_cache_list, PG_relation_prune_cache_list, full_relation_prune_cache_list, generate_answer_cache_list, reasoning_cache_list, force_answer_cache_list=get_cached_reponses()
    ...
    if cfg.relation_search_tool=="rel_entity_dict":
        dict_rel_file="/storage/ola/BeamQA/entity_relation_prediction.pkl"
        with open(dict_rel_file, 'rb') as file:
    ...
    for k in tqdm(range(len(datas))):
        ...
        for depth in range(1, cfg.depth+1):
            ...
            for entity_id,prev_rel  in topic_entity_relation:
                ...
                if cfg.relation_search_tool=="path_generation":
                    retrieve_relations_with_scores, LLM_calls = relation_search_prune_embed(PG_relation_prune_cache_list, beam_data, entity_id,entity_name, relation_list, question, cfg, prev_rel,depth-1,LLM_calls)  # best entity triplet, entitiy_id
                elif cfg.relation_search_tool=="llm":
                    retrieve_relations_with_scores, LLM_calls = full_relation_search_prune_embed(full_relation_prune_cache_list, entity_id,entity_name, relation_list, question, cfg,LLM_calls)  # best entity triplet, entitiy_id
                else:
                    if entity_id in relation_entity_prediction:                                                                                                                                    
                        relation_list=relation_entity_prediction[entity_id]
                    else: 
                        logging.info(f'\t\tentity: {entity_id} is not in the dictionary')
                    retrieve_relations_with_scores, LLM_calls = dictionary_relation_search_prune_embed(dictionary_relation_prune_cache_list, entity_id,entity_name, relation_list, question, cfg,LLM_calls)  # best entity triplet, entitiy_id
        ...
        if k%10==0:
            if PG_relation_prune_cache_list[1]>10 or  generate_answer_cache_list[1]>10 or reasoning_cache_list[1]>10 or full_relation_prune_cache_list[1]>10 or force_answer_cache_list[1]>10 or dictionary_relation_prune_cache_list[1]>10:
                logging.info(f"Dumping cache files: PG_relation_prune_cache_list:{PG_relation_prune_cache_list[1]}, generate_answer_cache_list: {generate_answer_cache_list[1]}, reasoning_cache_list: {reasoning_cache_list[1]}, force_answer_list: {force_answer_cache_list[1]}, full_relation_prune_cache_list: {full_relation_prune_cache_list[1]}, dictionary_relation_prune_cache_list:{dictionary_relation_prune_cache_list[1]}")
                save_responses(dictionary_relation_prune_cache_list[0],PG_relation_prune_cache_list[0],full_relation_prune_cache_list[0], generate_answer_cache_list[0], reasoning_cache_list[0], force_answer_cache_list[0])
                PG_relation_prune_cache_list[1]=0
                generate_answer_cache_list[1]=0
                reasoning_cache_list[1]=0
                force_answer_cache_list[1]=0
                full_relation_prune_cache_list[1]=0
        logging.info('+ ----------------------------------------------------------------- +')
