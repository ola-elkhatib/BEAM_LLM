...
def entity_search_embedding_space(head, rel,model,rel2idx,entity2idx,idx2entity,nx_graph,device,topk = 20 ,retscore = False):
    '''
...
def path_finder_rec(headname, relations,scorez,model,entity2idx,idx2entity,rel2idx,nx_graph,device,topk=10):
    '''
    ...
    for rel,pscore in zip(relations,scorez):
        prev_return = [(headname, 1)]
        for j , path_i in enumerate(path):
            prev_return = check_rec(prev_return, path_i, headname, topk, model, entity2idx, idx2entity,rel2idx, nx_graph, device)
